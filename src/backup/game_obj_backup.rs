...
pub struct GameObjCollection<'a> {
    map: HashMap<&'a str, usize>,
    pub objects: Vec< GameObj<'a> >,
}
impl<'a> GameObjCollection<'a> {
    pub fn from_iter( iter: IntoIter<(&'a str, GameObj<'a>)> ) -> Self  {
    ...
    pub fn get_object(&mut self, name: &str) ->  *const GameObj<'a> {
        return &self.objects[ self.map[name] ] as *const GameObj<'a> ;
    }
    pub fn get_mut_object(&mut self, name: &str) -> *mut GameObj<'a> {
        return &mut self.objects[self.map[name]] as *mut GameObj<'a>;
    }
}
...
impl<'a> GameObj<'a> {
    ...
    pub fn new(name: &'a str, collision_box: CollisionBox,  mesh: Mesh) -> Self {
        Self { name, collision_box, mesh}
    }
    ...
    pub fn static_sat(&mut self, other: &GameObj) {
        self.collision_box.static_sat(&other.collision_box);
    }

    pub fn draw_collision_box(&self, mat_proj: &Mat4x4, res:f64, scale:u32, color:[u8;4], fb: &mut FrameBuf) {
        self.collision_box.draw(mat_proj, res, scale, color, fb);
    }
...
impl CollisionBox {
    pub fn new(points: Vec<Vec3D>, faces: FaceInput) -> Self {
        ...
        CollisionBox { points, centre , edges: unique_edges, faces: box_faces }
    }
    ...
    pub fn draw(&self, mat_proj: &Mat4x4, res:f64, scale:u32, color:[u8;4], fb: &mut FrameBuf) {
        ...
        let project_unit = |p: &Vec3D| -> Vec3D {
            let p = Vec3D::new(p.x, p.y, p.z+3.0);
            let mut p = vec_multiply_mat(&p, &mat_proj);
            p
        };
        let project_shift = |p: &Vec3D| -> Vec3D {
            let mut p = Vec3D::new(p.x, p.y, p.z);
            // Shift into view
        ...
        // // Add projected points & apply clipping
        ...
        let plane = (Vec3D::new(0.,0.0,0.0), Vec3D::new(0.,0.,1.) ); // (point, normal)
        ...
        for f in &self.faces {
            ...
            for i in 0..n{
                ...
                let vec_a = v1; let vec_b= v2.sub_vec(&v1);
                ...
                if let Some( (intersect,t)) = vec_intersect_plane(&plane.0, &plane.1, &vec_a, &vec_b) {
                    ...
                    if t >= 0.0 && t <= 1.0 {
                        if d1 >= 0.0 {
                            draw::draw_line_2d(&project(&v1) , &project(&intersect), res, scale, color, fb);
                        }else {
                            // draw::draw_line_2d(&project(&intersect) , &project(&v2), res, scale, color, fb);
                        }
                    }else {
                ...
                //~~~~~~~~
                // if v1.dot(&plane.1) < 0.0 && v2.dot(&plane.1) < 0.0 { // both points  behind camera
                // }else if v1.dot(&plane.1) < 0.0  && v2.dot(&plane.1) >= 0.0 {
                    
                //     // //-------- Find intersect ----------------
                //     // let a = v1; // v1 -> v2
                //     // let b = v2.sub_vec(&v1);
                //     // let intersect = plane.0.sub_vec(&b.mult_scalar(a.dot(&plane.1) /  b.dot(&plane.1) )  );
                //     // //---------------------------------------
                //     draw::draw_line_2d(&intersect, &v2, res, scale, color, fb);
                // }else if  v1.dot(&plane.1) >= 0.0  && v2.dot(&plane.1) < 0.{
                ...
                //     // let intersect = plane.0.sub_vec(&b.mult_scalar(a.dot(&plane.1) /  b.dot(&plane.1) )  );
                //     // //---------------------------------------
                //     draw::draw_line_2d(&v1, &intersect, res, scale, color, fb);
                // }else {
        ...
        draw::draw_square(centre_proj.x, centre_proj.y, 1, color, fb);
    }
    ...
    pub fn static_sat(&mut self, other: &CollisionBox) -> bool {
        ...
        // Find min / max shadows using faces of object 1
        if !test_face_norms(&self.faces, &mut min_overlap, &mut min_overlap_dir) {return false};
        // Repeat but for object2 faces. We can use a pointer to simplify it
        if !test_face_norms(&other.faces, &mut min_overlap, &mut min_overlap_dir) {return false};
